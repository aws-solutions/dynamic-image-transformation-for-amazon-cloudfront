...
import { ILogGroup, LogGroup, QueryString } from "aws-cdk-lib/aws-logs";
...
// ============================================================================
// V8 CloudFront Function Tier Detection Metrics
// ============================================================================

// CF Function logs are always in us-east-1 regardless of stack region.
// The log group ARN is constructed explicitly with us-east-1.
export function addCFTierDetectionMetrics(
  this: SolutionsMetrics,
  props: {
    functionName: string;
    identifier?: string;
  ...
  const logGroup = LogGroup.fromLogGroupArn(
    this,
    `CFTierDetectionLogGroup${props.identifier || ""}`,
    `arn:aws:logs:us-east-1:${Aws.ACCOUNT_ID}:log-group:/aws/cloudfront/function/${props.functionName}:*`
  );
  // CF Function logs are prefixed with a request ID, preventing CWL auto-parsing of JSON fields.
  // Use filter on @message and parse to extract name explicitly.
  this.addQueryDefinition({
    logGroups: [logGroup],
    queryString: new QueryString({
      filterStatements: ['@message like "DIT-DETECT"'],
      parseStatements: [`@message '"name":"*"' as name`],
      stats: `sum(name = "ClientHintsWidth") as Tier1_ClientHintsWidth, sum(name = "ClientHintsViewportWidth") as Tier2_ClientHintsViewportWidth, sum(name = "JsCookie") as Tier3_JsCookie, sum(name = "CloudFrontDevice") as Tier4_CloudFrontDevice, sum(name = "Fallback") as Tier5_Fallback`,
    }),
    queryDefinitionName: `CFTierDetectionMetrics${props.identifier || ""}`,
  });
}
